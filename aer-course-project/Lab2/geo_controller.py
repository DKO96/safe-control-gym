...
class GeoController():
    ...
    def _compute_desired_force_and_euler(self,
        ...
        pos_e_max = np.array([0.6, 0.6, 0.3])
        vel_e_max = np.array([1.0, 1.0, 1.0])

        # remap errors if outside range
        pos_e = np.clip(pos_e, -pos_e_max, pos_e_max)
        vel_e = np.clip(vel_e, -vel_e_max, vel_e_max)
        ...
        kvy = 4 
        kvz = 2
        K_pos = np.diag((kpx, kpy, kpz))
        K_vel = np.diag((kvx, kvy, kvz))
        
        z_w = np.array([[0, 0, 1]])   

        a_fb = np.matmul(K_pos, pos_e) + np.matmul(K_vel, vel_e)
        a_des = a_fb + target_acc + self.grav*z_w
        ...
        xC = np.array([np.cos(reference_yaw), np.sin(reference_yaw), 0])
        yC = np.array([-np.sin(reference_yaw), np.cos(reference_yaw), 0])

        zB_des = a_des / np.linalg.norm(a_des)
        xB_des = np.cross(yC, zB_des) / np.linalg.norm(np.cross(yC, zB_des))
        yB_des = np.cross(zB_des, xB_des)

        R_des = np.vstack((xB_des, yB_des, zB_des)).T
        rotation = Rotation.from_matrix(R_des)
        desired_euler = rotation.as_euler('xyz', degrees=False)
